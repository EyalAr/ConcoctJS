module.exports = {
    ...
    initLinks: function(options, templates, contexts, links, buffers, log, done) {
        ...
        if (options.linkingRules) {
                ...
                async.parallel([
                    ...
                    contextRuleMatches.forEach(function(contextPath) {
                        templateRuleMatches.forEach(function(templatePath) {
                            ...
                            // only add link if both context and template exist (issue #2)
                            if (contexts[contextPath] && templates[templatePath]) {
            ...
            log.warn('No linking rules found.');
            done();
        }
            ...
            async.each(srcs, function(src, done) {
                ...
                fs.readFile(src, function(err, data) {
                    ...
                    try {
                        ...
                        log.warn('Invalid format in context file %s. Skipping.', src);
                    }
