	...
	readTemplates: function(options, templates, contexts, log, done) {
		...
		function _buildSources(src, srcs, done) {
			...
			if (typeof src === 'string') {
				...
				done('\'src\' field must be string or array. Got ' + typeof src);
		...
		async.series([
			...
			function(next) {
				...
				log.info('Reading templates...s');

				_buildSources(options.src, _srcs, function(err) {

					if (!err) {
						if (_srcs.length) {
							log.info('Found %d templates.', _srcs.length);
						} else {
							log.warn('No templates found in %s', options.src);
						}
					}
